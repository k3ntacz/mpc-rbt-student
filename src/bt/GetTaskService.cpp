...
{
public:
    GetTaskService(const std::string& name,
    ...
    static BT::PortsList providedPorts()
    {
        return providedBasicPorts({
            BT::OutputPort<std::string>("manipulator_id", "Assigned manipulator ID")
        });
    }
    ...
    BT::NodeStatus onResponseReceived(const Response::SharedPtr& response) override
    {
        ...
        // Vraťte SUCCESS.
        if (!response->success) {
            RCLCPP_ERROR(logger(), "GetTaskService: service returned failure");
            return BT::NodeStatus::FAILURE;
        }

        setOutput("manipulator_id", response->message);
        RCLCPP_INFO(logger(), "GetTaskService: manipulator_id = %s", response->message.c_str());
