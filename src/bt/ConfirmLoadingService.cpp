...
{
public:
    ConfirmLoadingService(const std::string& name,
    ...
    BT::NodeStatus onResponseReceived(const Response::SharedPtr& response) override
    {
        ...
        if (!response->success) {
            RCLCPP_WARN(logger(), "ConfirmLoadingService: Warehouse denied loading.");
            return BT::NodeStatus::FAILURE;
        }

        RCLCPP_INFO(logger(), "ConfirmLoadingService: Robot loaded successfully.");
